  ...
  return (
    <Layout>
      ...
      <div className="bg-[var(--bg-main)] min-h-screen pt-20 sm:pt-24 transition-colors duration-500">
        {/* ============ 🟢 SECTION 1: HERO & SYSTEM OVERVIEW (Legacy / Non-Featured) ============ */}
        {!manual.features && (
        ...
        {/* ============ 🟢 SECTION 2: SYSTEM LAYOUT & PARAMETERS (Legacy) ============ */}
        {!manual.features && (
        ...
        {manual.features && Array.isArray(manual.features[0]) === false && typeof manual.features[0] === 'object' && (
          <div className="bg-[var(--bg-main)]">
            ...
            <div className="container-content pt-12">
               ...
               <div className="mb-16">
                  <div className="inline-flex items-center gap-3 px-3 py-1 rounded-md bg-hejaaz-secondary/10 border border-hejaaz-secondary/20 mb-6">
                    <div className="w-1.5 h-1.5 rounded-full bg-hejaaz-secondary animate-pulse" />
                    <span className="text-[10px] font-black text-hejaaz-secondary uppercase tracking-[0.3em]">Module-Based System Architecture</span>
                  </div>
                  <h1 className="text-4xl sm:text-5xl lg:text-7xl font-black uppercase tracking-tight leading-[1.05]" style={{ color: 'var(--text-main)' }}>
                    {manual.name}
                  </h1>
               </div>
            </div>
              ...
              <section key={i} className={`py-12 sm:py-20 ${i % 2 !== 0 ? 'bg-[var(--card-bg)]' : 'bg-[var(--bg-main)]'} overflow-hidden border-b border-hejaaz-border/40`}>
                <div className="container-content">
                    ...
                    >
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {feature.subFeatures?.map((sub, j) => (
                          <div key={j} className="flex items-center gap-4 p-4 bg-hejaaz-secondary/[0.04] backdrop-blur-sm rounded-xl border border-hejaaz-secondary/20 shadow-sm transition-all hover:translate-x-1">
                            <div className="w-2 h-2 rounded-full bg-hejaaz-secondary shadow-[0_0_8px_rgba(242,121,34,0.3)]" />
                            <span className="text-[11px] font-black uppercase tracking-wider" style={{ color: 'var(--text-main)' }}>{sub}</span>
                          </div>
                        ))}
                      </div>
                    </motion.div>
                  ...
                  {feature.image && (
                    <motion.div 
                      {...fadeInSlideUp}
                      className="flex flex-col items-center justify-center p-8 bg-[var(--bg-secondary)] rounded-[3rem] border border-hejaaz-secondary/20 shadow-sm overflow-hidden"
                    >
                       <div className="relative group">
                         <div className="absolute inset-0 bg-hejaaz-secondary/5 blur-3xl opacity-0 group-hover:opacity-100 transition-opacity" />
                         <img 
                           src={feature.image} 
                           alt={`${feature.title} Unit`} 
                           className="max-h-[400px] w-auto object-contain relative z-10 transition-transform duration-700 group-hover:scale-110 mix-blend-multiply dark:mix-blend-normal"
                         />
                       </div>
                       <p className="text-[10px] font-black uppercase tracking-[0.4em] opacity-40 mt-8">Industrial hardware interface visual</p>
                    </motion.div>
                  )}
                  ...
                  {feature.specs && (
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-12 sm:mt-20">
                      {feature.specs.map((spec, k) => (
                        <div key={k} className="p-6 bg-hejaaz-secondary/[0.04] backdrop-blur-sm rounded-2xl border border-hejaaz-secondary/20 shadow-sm flex flex-col gap-2 hover:shadow-md transition-shadow">
                          <span className="text-[10px] font-black uppercase tracking-widest text-hejaaz-secondary">{spec.param}</span>
            ...
            <div className="max-w-4xl mx-auto">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4">
                {manual.specs?.map((spec, i) => (
                  <div key={i} className="flex justify-between items-center p-4 sm:p-6 bg-[var(--bg-secondary)] rounded-xl sm:rounded-2xl border border-hejaaz-secondary/20 shadow-sm transition-all hover:translate-x-1">
                    <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-wider sm:tracking-widest text-hejaaz-secondary/70">{spec.param}</span>
                    <span className="text-[10px] sm:text-xs font-black uppercase tracking-tighter" style={{ color: 'var(--text-main)' }}>{spec.value}</span>
                  </div>
                ))}
              </div>
            </div>
